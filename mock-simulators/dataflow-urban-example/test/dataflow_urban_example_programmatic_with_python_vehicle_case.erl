...
-module(dataflow_urban_example_programmatic_with_python_vehicle_case).

-moduledoc """
Programmatic test for the `Dataflow Urban Example` case, illustrating the
use of a processing unit implemented in Python.

The purpose of this module is to perform an overall, integrated testing of the
dataflow support taking into account the Python binding.
...
% In this simulation case, we have no third-party (for example a remote
% platform) to deal with, hence we do not need any specific identification
% server.
...
run() ->
    ...
    test_facilities:display( "Running a 'Dataflow Urban Example' simulation "
        ...
        "a Python-based processing unit.",
    ...
    SimulationSettings = #simulation_settings{
        simulation_name="Urban Dataflow Programmatic Python Example",
    ...
    DeploymentSettings = #deployment_settings{
        ...
        % In order to validate the Python binding API, a Python version of the
        % VehicleTypeUnit class has been designed and is tested here.
        %
        % For this purpose only, the Python binding process must be triggered at
        % the deployment level:
        %
        enable_language_bindings = [ python ] },
    ...
    DataflowPid = class_Actor:create_initial_actor( class_Dataflow,
        [ "Urban Programmatic Python Example Dataflow", ExperimentManagerPid ],
        LoadBalancerPid ),
    ...
    % Second, we consider the creation of two vehicle units, both based on an
    % actual Python model (whose corresponding code is to be found in
    % vehicle_type_unit.py):
    ...
    EnergyUnit = class_DataflowUnitManager:create_initial_unit(
        UrbanUnitManagerPid, class_EnergyDemandUnit, DataflowPid,
        [ "My Single Initial Energy Demand Unit" ] ),
