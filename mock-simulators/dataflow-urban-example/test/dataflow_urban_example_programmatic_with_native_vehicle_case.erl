...
% of a native processing unit (i.e. not relying on a binding, directy
% implemented in Erlang).
%
% The purpose of this module is to perform an overall, integrated testing of the
% native dataflow support.
%
% See also the 'Sim-Diasca Dataflow HOWTO'.
%
-module(dataflow_urban_example_programmatic_with_native_vehicle_case).
...
run() ->
	...
	SimulationSettings = #simulation_settings{
	  simulation_name="Urban Dataflow Programmatic Native Example",
	...
	DataflowPid = class_Actor:create_initial_actor( class_Dataflow,
			[ "Urban Programmatic Native Example Dataflow",
	...
	% actual native model (whose corresponding code is to be found in
	% class_VehicleTypeUnit.erl):

	VehicleUnits = [ FirstVehicleUnit, SecondVehicleUnit ] =
			class_DataflowUnitManager:create_initial_units(
			  UrbanUnitManagerPid,
			  % Same as { class_VehicleTypeUnit, erlang }:
			  class_VehicleTypeUnit,
			  DataflowPid,
			  % A vehicle type unit takes here four specific construction
			  % parameters: its name, its year of origin, its energy efficiency
			  % and its initial pollution (in)efficiency:
