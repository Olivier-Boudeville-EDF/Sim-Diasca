% Copyright (C) 20022-2022 Olivier Boudeville
%
...
% Note: this header is directly the class_TraceEmitter.hrl one, except that
% calls are redirected to the class_Traceable module (instead of the
% class_TraceEmitter one) that may in turn, if relevant, call the
% class_TraceEmitter services.
%
% This header cannot reuse the class_TraceEmitter one, as they define
% (differently) the same macros.


%% Start of the duplicated section.
...
% from a class_Traceable.

% See also traces.hrl, for the standalone sending of traces.


-ifndef(class_Traceable_hrl_guard).
-define(class_Traceable_hrl_guard,).
...
% Conventions section.
% (skipped as not relevant for a Traceable instance)
...
% For the most severe traces, we still use class_Traceable:sync/1 rather than
...
-endif. % class_Traceable_hrl_guard
