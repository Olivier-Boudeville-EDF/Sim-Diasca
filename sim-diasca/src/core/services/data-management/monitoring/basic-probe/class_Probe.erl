...
%
% - Test_probe.plot, with the relevant gnuplot commands
%
...
-doc "The name and options from which a probe can be constructed.".
-type name_options() :: probe_name_init()
                    | { probe_name_init(), probe_options() }.
...
-doc "The PID of a (basic) probe.".
-type probe_pid() :: class_ResultProducer:producer_pid().


-doc "The PID of a facility probe.".
-type facility_probe_pid() :: probe_pid().
...
-export_type([ probe_name/0, bin_probe_name/0, name_options/0,
               probe_tick/0,
               probe_name_init/0,
               probe_pid/0, facility_probe_pid/0, probe_ref/0,
               sample_data/0,
               probe_options/0,
...
-doc """
Constructs a basic probe, from the following parameters: NameOptions,
...
""".
-spec construct( wooper:state(), name_options(),
        [ declared_curve_name() ], [ declared_zone() ], title(),
        label(), label() ) -> wooper:state().
construct( State, NameOptions, CurveNames, Zones, Title,
           MaybeXLabel, YLabel ) ->
    construct( State, NameOptions, CurveNames, Zones, Title,
...
construct( State, _NameOptions={ NameInit, ProbeOptions }, CurveNames,
           Zones, Title,  MaybeXLabel, YLabel, MetaData,
           MaybeExtraSettingsTable, MaybeTickDuration )
                        when is_record( ProbeOptions, probe_options ) ->
...
-doc "Returns the directory where the probe files are to be written.".
-spec getProbeDirectory( wooper:state() ) ->
            const_request_return( option( bin_directory_path() ) ).
getProbeDirectory( State ) ->
    wooper:const_return_result( ?getAttr(probe_dir) ).
...
(sending one more value, or the `undefined` atom, for that curve).
""".
...
Note: all samples received afterwards are then expected to take it into account
(sending as many additional values, or the `undefined` atom, for these curves).
""".
...
sendResults( State, [ data_only ] ) ->
    ...
    ?debug_fmt( "Creating binary data-only archive term for ~p.",
                [ FileList ] ),
...
sendResults( State, [ rendering_only ] ) ->
    ...
    case ?getAttr(sample_count) of
        ...
        _ ->
            ...
            ?debug_fmt( "Creating binary plot-only raw term for ~p.",
                        [ ReportFilename ] ),
...
sendResults( State, [ data_and_rendering ] ) ->
    ...
    { FileList, ListedState } = case ?getAttr(sample_count) of
        ...
        0 ->
            % We do not generate the report, yet we used to want the command
            % file, even if it was not of use:

            %CommandState = ensure_command_file_available( State ),
            %ProducedState =
            %    setAttribute( CommandState, result_produced, true ),

            ProducedState = setAttribute( State, result_produced, true ),

            ?notice_fmt( "The probe '~ts' did not receive any data sample, "
                "hence does not yield a plot.", [ Name ] ),

            %{ [ PathLessDataFilename, CommandFilename ], ProducedState };
            { [ PathLessDataFilename ], ProducedState };
    ...
    ?debug_fmt(
        "Creating binary plot-and-data archive term for ~p, from '~ts'.",
        [ FileList, ProbeDir ] ),
...
-spec create_facility_probe( name_options(), [ declared_curve_name() ],
            [ declared_zone() ], title(), label(), label() ) ->
                                    static_return( facility_probe_pid() ).
...
""".
-spec delete_facility_probe( facility_probe_pid() ) -> static_void_return().
...
-doc """
Returns a `plot_settings` record with the specified information (expressed as
...
interpret_options( _ProbeOptions=#probe_options{
    ...
    { ProbeDir, MaybeBinProbeDir } = case ProbeDirectory of
        ...
        undefined ->
            ...
            % (.dat, .plot, .png) in the
            % '/tmp/sim-diasca-CASE*/deployed-elements' directory rather than
            % its 'outputs' sibling:
            %
...
Note however that [boxes may not be
relevant](https://gnuplot.sourceforge.net/docs_4.2/node241.html) for a proper
rendering of timestamped levels, in the sense that boxes will be rendered
*centered* about each specified abscissa, whereas generally one would want a
step starting exactly at that abscissa with the specified ordinate. `fillsteps`
is probably a lot more appropriate in this regard.
""".
...
generate_command_file( Name, Settings, CurveEntries, ZoneEntries,
    ...
    file_utils:write_ustring( File,
        ...
        % Not using 'noenhanced' anymore (preferring escaping the text):
        "set title \"~ts\"~n"
        ...
        [ PreambleStr,
          ...
          XrangeOpt,
          YrangeOpt,
          plot_utils:get_escaped_text( Settings#plot_settings.title ),
          plot_utils:get_escaped_text( Settings#plot_settings.x_label ),
          plot_utils:get_escaped_text( Settings#plot_settings.y_label ),
...
write_header( File, CurveEntries, ZoneEntries, Settings, Name, Metadata ) ->
    ...
    % Depending on the use, empty metadata may be transmitted (e.g. for the
    % performance tracker):
    %
    %trace_utils:debug_fmt( "Probe metadata: ~p", [ Metadata ] ),

    { SimNameBinStr, ShrunkMetadata } = list_table:extract_entry_with_default(
        _K=simulation_name, _Def= <<"(not set">>, Metadata ),

    MetadataAllStrings = [
        text_utils:format( "name of the simulation case: ~ts",
                           [ SimNameBinStr ] )
        | [ text_utils:binary_to_string( BinText )
                || { _Key, BinText } <- ShrunkMetadata ] ],

    MetadataString = text_utils:strings_to_string( MetadataAllStrings,
                                                   _Sep="# - " ),

    % Comments should ideally respect:
...
get_command_filename( Name ) ->
    file_utils:convert_to_filename_with_extension( Name, _Ext="plot" ).


-doc "Returns the gnuplot data filename.".
-spec get_data_filename( probe_name() ) -> file_name().
get_data_filename( Name ) ->
    file_utils:convert_to_filename_with_extension( Name, _Ext="dat" ).
...
get_report_filename( Name ) ->
    file_utils:convert_to_filename_with_extension( Name, _Ext="png" ).
    %file_utils:convert_to_filename_with_extension( Name, _Ext="svg" ).
...
generate_report( Name, State ) ->
    ...
    TargetFilePath = file_utils:ensure_path_is_absolute( TargetFilename ),

    file_utils:is_existing_file( TargetFilePath ) andalso
        begin
            ?warning_fmt( "The file '~ts' was already existing, "
                          "it has been removed.", [ TargetFilePath ] ),
            file_utils:remove_file( TargetFilePath )
        end,

    %file_utils:remove_file_if_existing( TargetFilePath ),
    ...
    % [  file_utils:ensure_path_is_absolute( CommandFilename ) ] ),
