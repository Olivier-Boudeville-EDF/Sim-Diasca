...
% Debugging notes:
%
% If a simulation seems stuck during deployment while waiting for a deployment
% report from at least one host manager, one may execute from that host
% (possibly the user one):
%
% more $(ls -1 -rt /tmp/sim-diasca-deployment-debug-for-sii-*.txt | tail -n 1)
...
-spec deploy( computing_host_manager_pid(), pid(), unit_utils:seconds(),
              ...
              language_utils:human_language(), error_report_output() ) ->
                    'onDatabaseStarted' | 'onDatabaseStopped'.
deploy( ComputerHostManagerPid, GroupLeaderPid, InterNodeTickTimeOut,
        BinDeployBaseDir, AdditionalBEAMBinDirs, SII, ErrorReportOutput,
        Lang ) ->

    % The sooner the better (as for the language setting, it must wait the
    % language_utils module to be deployed):
    %
    ...
    receive
        ...
        { wooper_result, send_starting } ->
            ...
            % Earliest possible moment (needing the corresponding module to be
            % deployed); necessary so that for example probes select the right
            % templates:
            %
            language_utils:set_user_language( Lang ),
...
final_main_loop( TraceAggregatorPid, ComputerHostManagerPid, BinDeployBaseDir,
    ...
    receive
        ...
        % user node (at least), and, should a disconnection happen, it will be
