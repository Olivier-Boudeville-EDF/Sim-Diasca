...
-define( class_attributes, [
    ...
    { error_report_output, basic_utils:error_report_output(),
      "the error report output to set on the computing node" },
...
- HostnameOptions={Hostname, Username}, a pair made of:

  - Hostname being either the name (as a plain string) of the remote computing
host to manage, or the 'localhost' atom

  - Username being the name of the user to
rely on for that host

- NodeOptions={NodeBaseName, NodeNamingNode, NodeCleanupWanted, NodeCookie,
NodeSchedulerCount}, a quintuplet made of:
...
- DeployOptions={DeploymentManagerPid, DeployTimeOut, InterNodeTickTimeOut,
BinDeployBaseDir, AdditionalBEAMDirs, SII}, a sextuplet where:
...
construct( State,
           ...
           _NodeOptions={ NodeName, NodeNamingNode, NodeCleanupWanted,
                          NodeCookie, NodeSchedulerCount, ErrorReportOutput },
    ...
    NonDottedMessageHostname = re:replace( _Subject=MessageHostname,
        _RegExp="\\.", _Replacement=":",
        _Opts=[ global, unicode, { return, list } ] ),
    ...
    ?send_info_fmt( StartingState,
        "Created a manager for computing host '~ts', "
...
connect_to_host( Hostname, State ) ->
    ...
    CompleteNodeName = ?getAttr(full_node_name),
                ...
                success ->
                    ...
                    case is_already_too_late( State ) of
                        ...
                        TimeOutString ->
                            ...
                            ?warning_fmt( "Deployment succeeded for host "
                                "'~ts', however it took too long compared "
                                "to the deployment time-out (~ts), "
                                "and thus this host has already been "
...
get_clean_up_command_for_host( Hostname, ScriptFullPath, State ) ->
    ...
    %UserHomeDirectory = text_utils:format( "/home/~ts", [Username] ),
    ...
    case ?getAttr(managed_host) of
        ...
        _Hostname ->
            ...
            case system_utils:run_command( CountCmd ) of
                ...
                { ReturnCode, CmdOutput } ->
                    ?error_fmt(
                        "Not able to establish whether node ~ts on host ~ts has"
                        " been successfully launched (code: ~B, message:'~ts')"
                        "Launch command was: '~ts'.",
...
get_erlang_launch_command( NodeName, Username, Hostname, State ) ->
    ...
    { BasicCommand, BasicEnv } = net_utils:get_basic_node_launching_command(
        CompleteNodeName, NamingMode, EpmdPort, ?getAttr(tcp_port_range),
        AdditionalOptions ),
...
send_deployment_agent( State ) ->
    ...
    % Of course we do not want to wait for this deploy/8 function to finish, as
    ...
    % 'Oneway' call of the deployment_agent:deploy/8 function on the target
    % computing node, corresponding to the creation of that agent:
    %
    rpc:cast( TargetNode, _SecondModule=deployment_agent,
              ...
              _SecondArgs=[ self(), GroupLeaderPid, ?getAttr(tick_time_out),
                            ...
                            language_utils:get_user_language() ] ),
