...
-define( class_attributes, [
    ...
    { simulation_instance_id, sii(), "stores the (possibly "
    ...
    { deployment_base_dir, directory_path(), "corresponds to the absolute "
      "base directory of the deployment to be done on each computing node" },

    { error_report_output, error_report_output(), "the selected setting for "
      "the simulation-level error report output (on all nodes)" },
    ...
    { deploy_time_out, milliseconds(), "the maximum number of "
    ...
    { graph_info, option( graph_info() ),
      "information regarding any graph stream service" },

    { web_manager_pid, option( web_manager_pid() ),
      "the PID of any web manager" },

    { root_data_exchanger_pid, option( data_exchanger_pid() ),
      "PID of any data exchanger" },
...
-export_type([ deployment_settings/0, manager_pid/0, node_type/0,
               ...
               host_list/0, element_path/0, element_type/0, element_option/0,
               element_spec/0, elements_to_deploy/0, firewall_options/0,
               language_binding_information/0, web_probe_classnames/0,
               computing_host_info/0, simulation_name/0 ]).


% Local types:

-type host_user_list() :: [ { string_host_name(), user_name() } ].
...
-type error_report_output() :: basic_utils:error_report_output().
...
construct( State,
    ...
    ErrorReportOutput = apply_error_report_output( DeploymentSettings ),
    ...
    % trace_utils:debug_fmt( "Engine root directory: ~ts.", [ EngineRootDir ] ),
    ...
    StartingState = setAttributes( ChangedState, [
        ...
        { error_report_output, ErrorReportOutput },
    ...
    ?send_info_fmt( StartingState,
        ...
        "~nVersions are:~n"
        " - for the Erlang user virtual machine: ~ts\n"
        " - for the Myriad layer: ~ts\n"
        " - for the WOOPER layer: ~ts\n"
        " - for the Traces layer: ~ts\n"
        " - for Sim-Diasca engine: ~ts\n"
...
construct( State, SimulationSettings, DeploymentSettings, LoadBalancingSettings,
    ...
    ?send_notice_fmt( TraceState, "Redeploying on following nodes: ~ts",
...
get_network_description() ->
    ...
    FileStrings = [ case file_utils:is_existing_file( Filename ) of
        ...
        true ->
            ...
            text_utils:format( "content of '~ts' is:~n"
                "\"\"\"~n~ts\"\"\"~n", [ Filename, FileContent ] );

        false ->
            text_utils:format( "(no '~ts' file found)", [ Filename ] )
    ...
    HostStrings = lists:foldl(
        fun( { Label, Opt }, AccStrings ) ->
            Cmd = text_utils:format( "~ts --~ts", [ HostnameCmd, Opt ] ),
            { _ReturnCode, CmdOutput } = system_utils:run_command( Cmd ),
            Optstring = text_utils:format( "the ~ts: ~ts",
                                           [ Label, CmdOutput ] ),
...
inspect_archive( ArchiveSelectedFiles, TargetPid ) ->
    ...
    { FilenameTable, ProblemFiles } = lists:foldl(
        fun( FullPath, _Acc={ TableAcc, PbFiles } ) ->
            ...
            %trace_utils:debug_fmt( " - inspecting '~ts'", [ FullPath ] ),
    ...
    receive
            ...
            case file_utils:is_leaf_among( ?typical_erlang_otp_beam_file,
                ...
                false ->
                    ?warning_fmt( "Following problematic files have been "
                        "detected in your simulation archive: ~ts",
                ...
                OTPFullPath ->
                    ?warning_fmt( "It looks like that at least a part "
                        ...
                        "simulation archive (the '~ts' file has "
                        "been spotted, in '~ts'). "
        ...
        { notify_archive_inspection, _DuplicateList=[ { Basename, FullPaths } ],
          _ProblemFiles } ->
            ...
            ?error_fmt( "The '~ts' file appears ~B times in the "
                "simulation archive, as: ~ts", [ Basename, length( FullPaths ),
        ...
        { notify_archive_inspection, DuplicateList, _ProblemFiles } ->
            ...
            ?error_fmt( "~B different filenames appear more than once in the "
                "simulation archive: ~ts", [ length( DupStrings ),
...
set_up_simulation_services(
    ...
    BindingManagers = set_up_binding_managers( LanguageBindings, EngineRootDir,
        ?getAttr(epmd_port), ServicePlacement, GraphState ),
...
shutdown_services( State ) ->
    ...
    class_InstanceTracker:is_local_tracker_registered() =:= not_registered
        orelse
            begin
...
process_setup_outcome( SimulationPackageFilename, State ) ->
    ...
    ?debug_fmt( "Waiting (as ~ts, with cookie '~ts') the deployment report "
        "from ~B host managers (~p), using a time-out of ~ts.",
...
wait_setup_outcome( Waited, Available, Failed, CollectTimeOut,
    ...
    receive
        ...
        { getPackage, [], HostManagerPid } ->
            ...
            ?debug_fmt( "Simulation package requested by host manager ~w.",
                        [ HostManagerPid ] ),
        ...
        { onHostDeploymentSuccess, [ HostManagerPid, HostInfos ] } ->
            ...
            ?info_fmt( "Received notification of set-up success from "
                ...
                "with user ~ts, for a target node named ~ts.",
                [ HostManagerPid,
        ...
        { onHostDeploymentFailure, [ HostManagerPid, Reason ] } ->
            ...
            ?warning_fmt( "Received notification of deployment failure "
                "from manager ~w: host '~ts' with user '~ts' "
                "will not take part to this simulation "
                "(reason: ~ts), its node (~ts) has been shut down.",
...
nodeup( State, NewlyConnectedNodeName ) ->
    ...
    ?debug_fmt( "A new node connected: '~ts'.", [ NewlyConnectedNodeName ] ),
...
nodedown( State, DisconnectedNodeName ) ->
    ...
    DownState = case ?getAttr(shutdown_initiated) of
        ...
        false ->
            ?emergency_fmt( "The '~ts' node disconnected, performing an "
...
onWOOPERNodeConnection( State, NewlyConnectedNodeName, MonitorNodeInfo ) ->
    ...
    % Presumably a computing node:
    ?debug_fmt( "A new node connected: '~ts' (monitor info: ~p).",
...
onWOOPERExitReceived( State, PidOrPort, ExitType ) ->
    ...
    case ?getAttr(shutdown_initiated) of
        ...
        false ->
            ...
            ?emergency_fmt( "Error exit signal received:~n ~p~nPerforming "
    ...
    case ?getAttr(database_running) of
        ...
        false ->
            ...
            case mnesia:delete_schema( DatabaseEnabledNodes ) of
                ...
                { error, { Reason, Arg } } ->
                    ?info_fmt( "No previous database deleted: ~ts: ~w.",
...
determine_temporary_directory( _DeploymentSettings=#deployment_settings{
        temporary_directory=Dir } ) when is_list( Dir ) ->
...
determine_temporary_directory( _DeploymentSettings=#deployment_settings{
        temporary_directory=InvDir } ) ->
    throw( { invalid_temporary_directory_specification, InvDir } ).
...
-doc """
Applies on this user node and returns the overall setting in terms of error
report output.
""".
-spec apply_error_report_output( deployment_settings() ) ->
                                        static_return( error_report_output() ).
apply_error_report_output( _DeploymentSettings=#deployment_settings{
        error_report_output=ErrorReportOutput } )
                                    when is_atom( ErrorReportOutput ) ->

    % Early check:
    case lists:member( _Elem=ErrorReportOutput,
                       basic_utils:get_all_error_report_outputs() ) of

        true ->
            % First, apply on this user node:
            basic_utils:set_error_report_output( ErrorReportOutput ),
            wooper:return_static( ErrorReportOutput );

        false ->
            throw( { invalid_error_report_output_specification,
                     ErrorReportOutput } )

    end;

apply_error_report_output( _DeploymentSettings=#deployment_settings{
        error_report_output=InvErrorReportOutput } ) ->
    throw( { invalid_error_report_output_specification,
             InvErrorReportOutput } ).
...
settings_to_string( _DeploymentSettings=#deployment_settings{
    ...
    PackageString = case PackageManager of
        ...
        { generate_and_save_deployment_package, TargetFilename } ->
            text_utils:format(
                "the simulation deployment package will be generated on-the-fly"
                " and saved in a deployment archive file named '~ts', ",
    ...
    TmpDirString = text_utils:format( "temporary directory used: '~ts'",
                                      [ TmpDir ] ),


    ErrorReportStr = text_utils:format( "error report output setting: '~ts'",
                                        [ ErrorReportOutput ] ),
    ...
    DataExchangerString = case EnableDataExchanger of
        ...
        { true, ConfigurationFiles } ->
            text_utils:format( "data-exchanger service enabled, with ~B "
                "configuration files specified: ~ts",
    ...
    wooper:return_static( text_utils:strings_to_string( [
        ...
        ErrorReportStr, DataloggerString, DataExchangerString,
        PerfTrackerString, BindingString, ResilienceString ] ) ).
...
-doc "Returns adequate meta-data for the result producers.".
get_result_metadata( _VersionStrs={ MyriadVersionStr, WOOPERVersionStr,
                                    TracesVersionStr, SimDiascaVersionStr },
                     SimulationName, TickDuration ) ->

    % Used for the web probes, so user-facing, and thus must use user language;
    % a side-effect is that it will be written in the probe data files as well,
    % whose content is currently (only) in English; class_probe:write_header/6
    % and al shall in the future be multi-language as well.

    UserLang = language_utils:get_user_language(),

    { MyriadVStr, WOOPERVStr, TracesVStr, SDVStr, TickFmtStr } =
        case UserLang of

            french ->
                { "version de la couche Myriad : ",
                  "version de la couche WOOPER : ",
                  "version de la couche Traces : ",
                  "version du moteur Sim-Diasca : ",
                  "durée d'un pas de temps (en secondes virtuelles) : ~f "
                      "(correspondant à ~ts)" };

            % Default: 'english':
            _ ->
                { "version of the Myriad layer: ",
                  "version of the WOOPER layer: ",
                  "version of the Traces layer: ",
    ...
    DurStr = time_utils:duration_to_string( _Milliseconds=1000*TickDuration,
                                            UserLang ),
    ...
    option_list:new( [
        ...
        { myriad_version, text_utils:string_to_binary(
            MyriadVStr ++ MyriadVersionStr ) },

        { wooper_version, text_utils:string_to_binary(
            WOOPERVStr ++ WOOPERVersionStr ) },

        { traces_version, text_utils:string_to_binary(
            TracesVStr ++ TracesVersionStr ) },

        { sim_diasca_version, text_utils:string_to_binary(
            SDVStr ++ SimDiascaVersionStr ) },

        % Any "name of the simulation case: " is best prefixed later, by the
        % caller (e.g. if wanting to emphasize on the name with a bold markup):
        %
        { simulation_name, text_utils:string_to_binary( SimulationName ) },

        { tick_duration, text_utils:string_to_binary( text_utils:format(
            TickFmtStr, [ TickDuration, DurStr ] ) ) } ] ).
...
create_data_exchangers( RootDataExchangerNode, ConfigurationFileList,
    ...
    _LocalExchangers = [ class_DataExchanger:remote_synchronous_timed_new_link(
        Node,
        _LocalName=text_utils:format( "Local exchanger for ~ts", [ Node ] ),
    ...
    case lists:member( UserComputingNodeName, AllExchangerNodes ) of
        ...
        false ->
            ...
            % to create a local data-exchanger specific to the user node:
            %
...
check_configuration_file_list( _FileList=[ Path | T ] ) when is_list( Path ) ->
    ...
    case text_utils:is_string( Path ) of
        ...
        false ->
            trace_bridge:error_fmt( "Invalid configuration filename "
                "(not a string): '~p'.", [ Path ] ),
...
check_configuration_file_list( _FileList=[ Elem | _T ] ) ->
    ...
    trace_bridge:error_fmt( "Invalid configuration filename (not a string): "
                            "'~p'.", [ Elem ] ),
    ...
    case DeploySettings#deployment_settings.perform_initial_node_cleanup of
        ...
        true ->
            ...
            case file_utils:is_existing_file( CleanScriptFullPath ) of
                ...
                false ->
                    ?emergency_fmt( "Script '~ts', used to remove any pending "
    ...
    Message = case Failed of
        ...
        [ { UniqueFailed, Reason } ] ->
            ...
            text_utils:format( "Error, the deployment on the single computing "
                "host specified, ~ts, failed (reason: ~ts), "
                "no simulation can be run.~n~ts",
        ...
        _ ->
            text_utils:format( "Error, no available computing node found "
                "on following ~B host candidates: ~ts~n~ts",
    ...
    case executable_utils:lookup_executable( EpmdExecName ) of
        ...
        false ->
            text_utils:format( "(no EPMD diagnosis available, "
                "as no '~ts' executable found)", [ EpmdExecName ] );

        EpmdPath ->

            Cmd = text_utils:format( "~ts -names", [ EpmdPath ] ),
            ...
            text_utils:format(
                "epmd, for port ~B, returned (with exit code ~B):~n~ts",
                [ EpmdPort, ReturnCode, CmdOutput ] )
...
get_epmd_local_diagnosis() ->
    ...
    text_utils:format( "local EPMD daemons found:~n~ts", [ ShellOutput ] ).
...
determine_host_list_from( { use_host_file_otherwise_local, HostFile,
    ...
    case file_utils:is_existing_file_or_link( HostFile ) of
        ...
        true ->
            ?notice_fmt( "Host file '~ts' found, using it.", [ HostFile ] ),
            get_hosts_from_file( HostFile, UserName );

        false ->
            ?notice_fmt( "No host file '~ts' found, defaulting to "
...
determine_host_list_from( UnexpectedHostInfo, _UserName, State ) ->
    ?error_fmt( "Invalid computing host specification: ~p.",
                [ UnexpectedHostInfo ] ),
...
ensure_username_specified( _HostList=[ E={ Host, UserName } | _T ],
                           _DefaultUserName, _Acc ) when is_atom( Host ) ->

    % Then UserName is not an atom:
    trace_bridge:error_fmt( "Invalid entry (~p) in host list: "
        "user name should be an atom, not '~p'.", [ E, UserName ] ),

    throw( { invalid_host_list_entry, { user_name_not_atom, UserName, E } } );


ensure_username_specified( _HostList=[ E={ Host, UserName } | _T ],
        _DefaultUserName, _Acc ) -> % Implicit: when is_atom( UserName ) ->

    % Then Host is not an atom:
    trace_bridge:error_fmt( "Invalid entry (~p) in host list: "
        "host should be an atom, not '~p'.", [ E, Host ] ),
...
ensure_username_specified( [ H | _T ], _DefaultUserName, _Acc ) ->
    trace_bridge:error_fmt( "Invalid entry (~p) in host list.", [ H ] ),
...
set_up_computing_nodes( BaseNodeName, HostUserList, InterNodeSeconds,
    ...
    %
    NodeOpts = { BaseNodeName, NamingMode, ?getAttr(node_cleanup),
                 erlang:get_cookie(), ?getAttr(compute_scheduler_count),
                 ?getAttr(error_report_output) },
    ...
    HostFun = fun( Hostname ) ->
        ...
        case Hostname of
            ...
            LocalHost ->
                %trace_utils:debug_fmt( "Localhost detected ('~ts').",
                %                       [ LocalHost ] ),
                localhost;

            H ->
                %trace_utils:debug_fmt( "Remote host detected ('~ts').",
                %                       [ H ] ),
                H
    ...
    HostInfos = [ #computing_host_info{
        ...
        node_name=net_utils:get_complete_node_name( BaseNodeName, Host,
                                                    NamingMode ),
...
check_node_availability_tolerance( #deployment_settings{
        ...
        when T =:= allow_unavailable_nodes
             orelse T =:= fail_on_unavailable_node ->
    T;
...
check_data_logger_wanted( #deployment_settings{ enable_data_logger=Other } ) ->
    throw( { invalid_data_logger_settings, Other } ).




% To avoid "The pattern {'deployment_settings'...} can never match since
% previous clauses completely covered the type #deployment_settings":
%
...
Returns a more canonical form of these settings.
""".
...
check_graph_streaming_wanted( #deployment_settings{
    ...
    throw( { invalid_graph_streaming_settings, Other } ).
...
% (helper)
-spec check_language_binding_setting( term(), [ language() ] ) -> boolean().
check_language_binding_setting( Language, SupportedLanguages )
                                    when is_atom( Language ) ->
    lists:member( Language, SupportedLanguages ) orelse
...
check_language_binding_setting( { Language, CodePath }, SupportedLanguages )
    ...
    text_utils:are_strings( CodePath ) orelse
...
% To avoid "The pattern {'load_balancing_settings'...} can never match since
% previous clauses completely covered the type #load_balancing_settings":
%
...
set_up_instance_tracking( TroubleShootingMode, #service_placement{
    ...
    LocalInstanceTrackers = [
        class_InstanceTracker:remote_synchronous_timed_new_link(
            Node, _ParentlInstanceTrackerPid=RootInstanceTrackerPid,
...
set_up_web_management( SII, EngineRootDir, InteractivityMode, ResultManagerPid,
    ...
    % deemed in this case able to launch any local webserver / browser); linked,
    % timed and synchronous, yet not waiting for any launched webserver to be up
    % and ready, as this would be a bit complex (HTTP connection polling), may
    % be long, and at worse the user of the browser may poll by reloading until
    ...
    %trace_utils:debug_fmt( "Result directory: '~ts'.", [ ResultDir ] ),

    WebserverContentRoot = class_WebManager:get_web_content_root( ResultDir ),

    % Created locally, on the user node:
...
set_up_binding_managers( LanguageBindings, RootDir, EpmdPort,
    ...
    Languages =:= [] orelse
        begin
            ?info_fmt( "Waiting for the managers of following ~B bindings: ~ts",
                [ length( Languages ),
...
set_up_performance_tracking(
    ...
    PerfTrackPid = class_PerformanceTracker:synchronous_timed_new_link(
        ...
        ?performance_tracker_registration_scope, ResultDirInfo ),
...
save_simulation_package( BinaryPackage, TargetFilename, State ) ->
    ...
    file_utils:is_existing_file_or_link( TargetFilename ) andalso
        % This happens typically if the previous run crashed; a warning
        % would be a lot:
        %
        ?info_fmt( "Overwriting an already-existing '~ts' "
            "simulation package.",
...
build_simulation_package( State ) ->
    ...
    WebDeploySettings =
        ...
        { true, WebProbeClassnames, _TCPPort, _WebserverInstallRoot } ->
            ...
            % Note that:
            %
            % - these are necessarily static methods (no instance
            % created yet), hence they are stateless
            %
            % - they are necessary to check the build - but they must be already
            % built (otherwise a clear 'undef' error will be reported)
            %
            AddedElems = lists:foldl(
                fun( Classname, Elems ) ->
                    case code_utils:is_beam_in_path( Classname ) of
                        ...
                        not_found ->
                            ?error_fmt( "Webmanager enabled, yet no BEAM "
                                "file found for ~ts.", [ Classname ] ),
                            throw( { web_classname_not_found, Classname } );

                        _Paths ->
                            %?debug_fmt( "Found as ~p.", [ Paths ] ),

                            % They may in turn call modules that might not be
                            % available; not a problem, as a clear 'undef' error
                            % will be reported:
                            %
                            Classname:get_elements_to_deploy( BinRootDir )
                                ++ Elems
    ...
    RuleString = text_utils:format( "Following ~B deployment selection rules "
        "have been applied: ~ts", [ length( CanonizedAdditions ),
            text_utils:strings_to_string( [ text_utils:format( "rule ~p",
                            [ R ] ) || R <- CanonizedAdditions ] ) ] ),
...
translate_path( Path, RootDir, RootDirLen, CurrentDir ) when is_list( Path ) ->
...
process_element( { ElementPath, _ElementType=data, ElementOptions }, State ) ->
    ...
    case file_utils:resolve_type_of( ElementPath ) of

        regular ->
            MustRebuild andalso rebuild_file( ElementPath, State ),
            [ ElementPath ];


        directory ->
            MustRebuild andalso rebuild_directory( ElementPath, State ),
...
process_element( { ElementPath, _ElementType=code, ElementOptions }, State ) ->
    ...
    %trace_utils:debug_fmt( "Processing code in ~ts with options ~p.",
    %                       [ ElementPath, ElementOptions ] ),
    ...
    case file_utils:resolve_type_of( ElementPath ) of
        ...
        directory ->
            ...
            %trace_utils:debug_fmt( "CodeContent for directory '~ts': ~ts",
        ...
        _Other ->
            ...
            ?error_fmt( "An additional code element path to deploy (~p) was "
                "specified, however it is not a regular file or a directory, "
                "neither a symbolic link, aborting deployment.",
                [ ElementPath ] ),
    ...
    BlackFileList = case proplists:get_value( exclude_directories, Options ) of
        ...
        ExcludedDirectories ->
            ...
            [ begin
                ...
                file_utils:is_existing_directory_or_link( AbsDir ) orelse
                    begin
    ...
    case system_utils:run_command( Command ) of
        ...
        { _ReturnCode=0, CmdOutput } ->
            ?info_fmt( "Directory '~ts' successfully rebuilt, based on "
                "following command: '~ts', from following directory: "
                "'~ts' and with following output:~n~ts",
        ...
        { ReturnCode, CmdOutput } ->
            ...
            DisplayedMessage = text_utils:format(
                "Rebuild of directory '~ts' failed (error code: ~B). "
                "Build error message was:~n~n\"\"\"~n~ts~n\"\"\"",
...
rebuild_file( Filename, State ) ->
    ...
    case system_utils:run_command( Command ) of
        ...
        { _ReturnCode=0, _CmdOutput=[] } ->
            ?debug_fmt( "File ~ts successfully rebuilt.", [ Filename ] );

        { _ReturnCode=0, CmdOutput } ->
            ?warning_fmt( "File ~ts successfully rebuilt, yet a message "
                          "was output: '~ts'.", [ Filename, CmdOutput ] );

        % Often code is 2:
        { ReturnCode, CmdOutput } ->

            DisplayedMessage = text_utils:format( "Rebuild of file ~ts failed "
                "(error code: ~B). Build error message was:~n~n'~ts'.",
                [ Filename, ReturnCode, CmdOutput ] ),

            ?error( DisplayedMessage ),

            %trace_utils:debug_fmt( "~n~ts", [ DisplayedMessage ] ),
...
get_result_directory_name( SimulationName, StartTimestamp, SII ) ->
    ...
    %
    wooper:return_static( text_utils:format( "~ts-on-~ts-by-~ts-~ts", [
...
placement_description( ServiceName, _Nodes={ RootNode, LocalNodes } ) ->
    text_utils:format( "~ts: root on ~ts, ~B agents on ~p",
...
halt_on_error( Message, ErrorCode ) when ErrorCode > 0 ->
    ...
    basic_utils:display( "~nHalting on error (code: ~B): ~ts",
                         [ ErrorCode, Message ] ),
