...
-type actor_pid() :: wooper:instance_pid().
        ...
        begin
            ...
            lists:min( AddedTicks ) > CurrentTickOffset orelse
                begin
                    ...
                    trace_bridge:error_fmt( "The following spontaneous tick "
                        "offets to add are not in the strict future of the "
                        "simulation, which is currently at "
                        "tick offset #~B: ~w.",
                        [ CurrentTickOffset, FaultyAddedTicks ] ),
        ...
        begin
            ...
            lists:min( WithdrawnTicks ) > CurrentTickOffset orelse
                begin
                    ...
                    trace_bridge:error_fmt( "The following spontaneous tick "
                        "offsets to withdraw are not in the strict future of "
                        "the simulation, which is currently at "
                        "tick offset #~B: ~w.",
                        [ CurrentTickOffset, FaultyWithdrawnTicks ] ),
...
-doc """
Checks the consistency of diascas, when an actor begins a new diasca.
...
execute_actor_oneway_as( Classname, Onewayname, State ) ->
    % Just a WOOPER call:
    executeOnewayAs( Classname, State, Onewayname ).
...
execute_actor_oneway_as( Classname, Onewayname, MethodArgs, State ) ->
    % Just a WOOPER call:
    executeOnewayAs( Classname, State, Onewayname, MethodArgs ).
