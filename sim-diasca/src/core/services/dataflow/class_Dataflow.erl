...
-spec create_channel_value( actual_value(), user_value_semantics(),
    unit_utils:unit_bin_string() | value_unit(), value_text_type() ) ->
                                    static_return( channel_value() ).
create_channel_value( ActualValue, UserSemantics, Unit, TextType )
...
create_channel_value( ActualValue, UserSemantics, UnitString, TextType )
...
create_channel_value( ActualValue, Semantics,
        Unit={ _UnitBinString, _CanonicalUnit }, TextType ) ->
    ...
    CtxType = case type_utils:parse_type( TextType ) of

        { ok, ContextualType } ->
            ContextualType;

        { error, TaggedErrorInfo } ->
            trace_bridge:error_fmt( "Unable to parse type '~p' "
                "for channel value '~p': ~ts",
                [ TextType, ActualValue,
                  type_utils:interpret_parse_type_error( TaggedErrorInfo ) ] ),

            % Includes TextType:
            throw( { channel_value_parse_failed, TaggedErrorInfo } )

    end,

    ChannelValue = #channel_value{ actual_value=ActualValue,
        semantics=Semantics, unit=Unit, type=CtxType },

    wooper:return_static( ChannelValue );


create_channel_value( _ActualValue, UserSemantics, Unit, _TextType ) ->
...
pre-processed semantics (set of binaries), unit (a pair) and contextual type
(i.e. type-as-term, rather than a type-as-a-string) for that (e.g. the one
already associated to a given port).
...
create_direct_channel_value( ActualValue, Semantics,
        Unit={ _UnitBinString, _CanonicalUnit }, CtxtType ) ->

    ChannelValue = #channel_value{ actual_value=ActualValue,
        semantics=Semantics, unit=Unit, type=CtxtType },
