...
%
% If nested creations are to be done and creations cannot be performed just
% after the simulation start (typically then during the first diasca),
% reproducibility can be preserved by limiting the number of creator processes
% to 1. Refer to the use of the
...
-doc "A full construction line being examined (as a binary).".
...
-doc "The full information necessary to create an instance.".
-type creation_spec() :: { classname(), [ method_argument() ],
                           class_LoadBalancer:placement_hint(), line_number() }.


-doc "Full context of a parsed creation line (file origin, line number, etc.).".
-type line_context() ::
...
""".
-type id_table() ::
    table( user_identifier(), { 'blank_pid', actor_pid() } | actor_pid() ).
...
-doc """
Returns any suitable user_id reference, as a string, from the specified user
identifier.

For example, `get_maybe_user_id_reference_for("John") = "{user_id, \"John\"}".`.
""".
...
manage_initialisation( InitialisationFiles, NodeCount, LoadBalancerPid ) ->
    ...
    CreatorsCount =:= 1 orelse
        ?notify_warning_fmt( "Relying on multiple (~B) instance creator(s), "
            "so in case of nested actor creations AAI allocations will not be "
            "fully reproducible.", [ CreatorsCount ] ),
...
%  the work to the next processing step, which is a lot more parallel (and,
%  hopefully, long/demanding)
%
...
read_init_file( FilePath, Creators, InstanceLoaderPid ) ->
    ...
    % binary: to support larger files we do not want to saturate the RAM, and
    % anyway we will process it on a per-line basis (and lower-level caching is
    % to happen anyway).
    %
...
-doc """
Reads all lines of the specified file, by chunks of the specified size, and
feeds them to creators as early as possible.
""".
...
-doc """
Tells whether the specified logical line is autonomous (complete), that is
whether it should be interpreted as a full (non-truncated) creation line, or it
shall be ignored.
        ...
        { process_chunk, _Chunk={ LineInfos, BinFilename }, ReaderPid } ->
            ...
            case basic_utils:wait_for_acks_nothrow( FilteredWaited,
                ...
                TimedOutPids ->
                    ...
                    % Interleaving:
                    IdResolverPid ! { report_blank_ids, self() },
                    ...
                    trace_bridge:error_fmt( "Time-out (after ~ts) "
                        ...
                        "instances, whose construction is at least long, if "
                        "not non-terminating: ~ts~n~ts",
...
line, possibly resulting in an instance creation and in various blank processes
to be created.
...
-doc """
Returns `{BinUserId, ActualCreationLine}` from the specified string, akin to:
...
-doc """
Explores and transforms the specified construction arguments, replacing
...
-doc """
Transforms the specified argument, replacing `{user_id,Id}` by the corresponding
PID to be used, and updating the first user identifier found.
...
-doc "Reports the specified error while parsing the specified creation line.".
...
- `IdTable :: id_table()` records th processes associated to user identifiers

- `InstanceLoaderPid :: pid()`, the PID of the instance loader process, as it
must be notified of termination
""".
-spec user_identifier_resolver_loop( id_table(), loader_pid() ) -> no_return().
user_identifier_resolver_loop( IdTable, InstanceLoaderPid ) ->
    ...
    receive
        ...
        % Reports the state w.r.t. identifiers, to facilitate debugging:
        { report_blank_ids, CallerPid } ->
            report_blank_ids( IdTable, CallerPid ),
            user_identifier_resolver_loop( IdTable, InstanceLoaderPid );
...
-doc """
Declares the specified user identifier.

Returns an updated identifier table, and sends back a `pid_assigned` message.
""".
-spec declare_id( user_identifier(), node(), id_table(), load_balancer_pid(),
                  pid() ) -> id_table().
...
""".
-spec resolve_id( user_identifier(), id_table(), load_balancer_pid(),
                  pid() ) -> id_table().
resolve_id( BinId, IdTable, LoadBalancerPid, CallerPid ) ->
...
-doc """
Reports the identifiers that still point to a blank PID, in order to facilitate
debugging.

Notably useful whenever an instance referenced in its creation line a bogus user
identifier (typo/mistake), as otherwise its creation just times out, with no
hint about a faulty user identifier (a blank PID being just created for this
bogus id).
""".
-spec report_blank_ids( id_table(), pid() ) -> void().
report_blank_ids( IdTable, CallerPid ) ->

    % Quite similar to next wrap_up_and_terminate/1:

    CheckFun = fun

        ( { BinId, { blank_pid, _BlankPid } }, { IdAcc, IdCount } ) ->
            { [ BinId | IdAcc ], IdCount+1 };


        ( { _BinId, _Pid }, { IdAcc, IdCount } ) ->
            { IdAcc, IdCount+1 }

               end,

    ReportStr = case table:fold_on_entries( CheckFun,
            _Acc0={ _IdAcc=[], _IdCount=0 }, IdTable ) of

        { [], TotalCount } ->
            text_utils:format( "(all ~B user identifiers have been assigned "
                               "to an actor)", [ TotalCount ] ) ;

        { [ UndefinedId ], TotalCount } ->
            text_utils:format( "Note: among the ~B known user identifiers, "
                "one has been referenced yet has not been created, short of "
                "having been defined: '~ts'.",
                [ TotalCount, UndefinedId ] );

        { UndefinedIds, TotalCount } ->
            text_utils:format( "Note: among the ~B known user identifiers, "
                "~B have been referenced yet have not been created, short of "
                "having been defined: ~ts",
                [ TotalCount, length( UndefinedIds ),
                  text_utils:strings_to_string( UndefinedIds ) ] )

    end,

    CallerPid ! { blank_ids_report, ReportStr }.




-doc "Finishes the instantiation.".
-spec wrap_up_and_terminate( id_table() ) -> void().
wrap_up_and_terminate( IdTable ) ->

    % Let's scan the table to ensure that no blank_pid remains (the accumulator
    % will be the list of still undefined user identifiers):

    %trace_utils:debug_fmt( "Final identifier table: ~ts",
    %                       [ table:to_string( IdTable ) ] ),
