...
- MeshOptions:
  ...
  - a graph layout may be chosen (see `mesh_layout/0`)
...
Each added node can be either a simple node `N`, or a `{N,
AssociatedNodeContent}` pair.
""".
...
displayRenderingIn( State, TargetDirectoryPath ) ->
    ...
    RenderingFilename =
        file_utils:convert_to_filename_with_extension( Basename, "png" ),
...
Mostly the same as `format_options/1`, except that some options are overridden
for nodes.
""".
