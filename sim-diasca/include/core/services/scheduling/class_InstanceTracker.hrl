...
%
% The actor PID is not stored there, as each actor record will be actually a
% value associated to a key, which will be the PID of that actor.
%
-record( actor_info, {
...
-doc """
Abstract (as opposed to a PID) reference to an agent of a simulation service on
a given node (akin to the actor's AAI).
