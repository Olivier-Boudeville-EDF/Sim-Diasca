...
% of the various main services involved):
%
...
-doc """
Publicly, directly available state of the dataflow subsystem (mainly the PIDs of
the main various services involved).
""".
...
-doc """
Name of a port: an arbitrary non-empty binary string, unique among all ports and
iterations of the same direction - input or output - of a dataflow element, and
containing the ?iterated_port_token substring iff it is an iterated port.
""".
-type port_name() :: text_utils:bin_string().


-doc """
String-based port name, typically for user input (converted then internally to a
binary string); an arbitrary non-empty string, unique among all ports and
iterations of the same direction - input or output - of a dataflow element, and
containing the ?iterated_port_token substring iff it is an iterated port.
""".
...
-doc """
Name of a port iteration (arbitrary non-empty string, unique among all ports and
iterations of the same direction - input or output - of a dataflow element, and
not containing the ?iterated_port_token substring).
""".
...
-doc "To count port instances.".
-type port_count() :: basic_utils:count().


-doc "A minimum number of ports.".
-type min_port_count() :: port_count().


-doc "A maximum number of ports.".
-type max_port_count() :: port_count() | 'unbounded'.


-doc "Both endpoints of a channel, in the context of two blocks.".
...
-doc "A comment, once stored for good in any kind of actor.".
-type internal_comment() :: text_utils:bin_string().



-doc "A context-free identifier of a port.".
-type full_port_id() :: { block_pid(), port_type(), port_name() }.



-doc """
Port source, to designate the source endpoint of a channel, i.e. the output port
...
-doc """
Port target, to designate the target endpoint of a channel, i.e. the input port
...
-doc "Name of an input (port) iteration.".
...
-doc "Name of an output (port) iteration.".
...
-doc """
Iteration port source, to designate a port iteration that is the source endpoint
for a set of channels: defines the iterated output ports meant to target
upstream blocks.
""".
...
-doc """
Iteration port target, to designate a port iteration that is the target endpoint
of a set of channels: defines the iterated input ports meant to target
downstream blocks.
...
-doc """
Unitary element (concept) comprised in a semantics, as expressed by the user; it
shall be a RDF subject, i.e. an IRI ('International Resource Identifier').

For example: "http://dbpedia.org/resource/Leonardo_da_Vinci",
"http://xmlns.com/foaf/0.1/knows", to be preferred to "energy_demand", "unitary
cost", "Number of tracks elapsed", etc.
...
-doc """
Unitary element (concept) comprised in a semantics, when used internally by the
engine; it is expressed as a RDF subject, i.e. an IRI (`International Resource
Identifier`).
...
(typically display), in our standard form; e.g. `km/h` or `mW.K^2.m^-3` (in the
...
Please refer to the section about units in the technical manual of the 'Myriad'
...
-doc """
An (absolute) identifier of an input port (no port type useful in this context).
""".
...
-doc "Identifier of any kind of port (input or output).".
...
whereas its associated block was suspended (thus the set value is to be sent to
...
-doc """
Concentrates all information (mandatory or not) that may be provided by the user
when defining an input port.
""".
...
-doc """
Value exchanged over a dataflow channel, typically emitted by an output port and
received by an input port.
""".
...
-doc """
Name of a dataflow attribute, that is a dataflow-visible attribute of a dataflow
object.
...
-doc "Name of a dataflow attribute (as a binary string).".
-type dataflow_attribute_bin_name() :: port_name().



-doc "Comment associated to a dataflow attribute.".
...
-doc """
Type of a dataflow object, that is its classname (e.g. `'class_FoobarObject'`).
""".
-type dataflow_object_type() :: block_type().


-doc """
Type of a processing unit, that is its classname (e.g. `'class_FoobarUnit'`).
""".
...
-doc """
Name of a dataflow object manager, that is its classname (e.g.
...
-doc """
Name of a dataflow unit manager, that is its classname
(e.g. `'class_UrbanUnitManager'`).
""".
-type unit_manager_name() :: wooper:classname().


-doc "Definition of the valid formats for mockup clauses.".
...
-doc """
Downstream block, that is regarded as having input ports involved in a channel.
""".
-type downstream_block_pid() :: block_pid().


-doc "PID of a dataflow mockup unit.".
...
-doc "PID of the identification server.".
