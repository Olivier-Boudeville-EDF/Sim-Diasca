...
-record( deployment_settings, {
    ...
    % Forces any (distributed, consistent) setting for the output of error
    % reports.
    %
    % If not wanting longer reports to be truncated, it may be set to
    % 'standard_full'.
    %
    error_report_output =
        standard_ellipsed :: basic_utils:error_report_output(),
    ...
    % Setups the webmanager if requested, in order to enable the browsing of
    % simulation-related web content (typically HTML-based results; possibly, in
    % some future, in order to control the simulation).
    %
    ...
    %
    % - the TCP port at which any webserver shall run can be specified
    ...
    %
    % (the last two parameters are currently ignored, as no webserver is
    % currently used)
    %
