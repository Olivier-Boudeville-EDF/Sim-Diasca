...
-module(gui_styled_editor_test).

-moduledoc """
Unit tests for the management of the **GUI styled text editor**.
""".
...
run_gui_test() ->
    ...
    test_facilities:display( "~nStarting the styled text editor test." ),
    ...
    Editor = gui_styled_text_editor:create(
        _Opts=[ { text, "This is the initial text" },
                { style, TextEditorStyle } ],
        _ParentWin=Frame ),

    gui_styled_text_editor:declare_style( Editor, my_warning_style, [ bold,
        underline, { foreground_color, red }, { background_color, yellow } ] ),


%% %% Appliquer style 1 sur "Hello"
%% wxStyledTextCtrl:startStyling(Stc, 0, 0),
%% wxStyledTextCtrl:setStyling(Stc, 5, 1).

%% wxStyledTextCtrl:setLexer(Stc, ?wxSTC_LEX_ERLANG),

%% %% Mots-clés
%% wxStyledTextCtrl:setKeyWords(Stc, 0,
%%   "after begin case catch cond end fun if let of query receive try when"),

%% %% Styles
%% wxStyledTextCtrl:styleSetForeground(Stc, ?wxSTC_ERLANG_COMMENT, {0,128,0}),
%% wxStyledTextCtrl:styleSetForeground(Stc, ?wxSTC_ERLANG_KEYWORD, {0,0,255}),
%% wxStyledTextCtrl:styleSetForeground(Stc, ?wxSTC_ERLANG_VARIABLE, {128,0,128}).
