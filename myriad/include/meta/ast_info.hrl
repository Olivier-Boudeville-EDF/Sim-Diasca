...
%
% In the former case, a single field (e.g. 'module') contains a pair of
...
%
% - the high-level, developer-friendly one (e.g. 'compilation_options')
% - the raw one in AST form (e.g. 'compilation_option_defs')
%
...
-record( module_info, {
	...
	% Note:
	%
	% - for this aggregated, higher-level field, the specific way that was used
	% in order to specify these various information (e.g. separately or in
	% groups, at which actual file locations, etc.) is abstracted out
	%
	% - options can also be specified through the command line with the -D
	% selector (e.g. see ERLANG_COMPILER_TOKEN_OPT); as the in-source ones, they
	% are gathered in this table (yet do not have of course counterpart forms in
	% the compilation_option_defs field below); no specific precedence between
	% command-line options applies, as to a given option name is associated a
	...
	% The actual, lower-level definitions of the compilation options, like in:
	%
	% [...]
	...
	% associating, to an attribute name (an atom key such as 'my_attribute'), a
	% list of pairs comprising each a value (e.g.  my_value) and the
	% corresponding AST form (knowing of course that a given attribute name may
	...
	%
	% Unlike the raw definitions (in the 'include_defs' field), this field does
	% not include the filename of the module being compiled (e.g.
	% "foobar.erl").
	%
	% (there is no duplicate either in that include list; we do not use a set
	% here though, as set_utils is not a bootstrapped module)
	%
	% Note: to be kept in sync with its 'include_defs' counterpart lower-level
	% field.
	%
	...
	% Include definitions:
	%
	% (possibly a given file might be included more than once; it is generally
	% the case for the module being currently compiled)
	%
	...
	% is stored primarily in its own type_info record (see the 'types' field)
	% through a list of locations, while the information sufficient to
	% reconstruct the actual forms for the exports of all types are recorded
	% here.
	%
	...
	% definition.
	%
	% Note: this field must be kept synchronised with the table in the 'types'
	% field.
	%
	...
	% Lists the functions imported by that module, indexed per implementation
	% module (each being a key of that table).
	%
	...
	% while the information sufficient to reconstruct the actual forms for the
	% exports of all functions are recorded here.
	%
	...
	% among its locations (in its 'exported' field) the location of an actual
	% export form (typically the one of the previous marker) in this current
	% field (function_exports), the corresponding export will be added here
	% automatically, if not already defined.
	%
	function_exports :: ast_info:function_export_table(),


	% All information, indexed by function identifiers, about all the functions
	...
	%
	% or forms meant to trigger errors (e.g. if resulting in defining a function
	% more than once)
	%
...
-record( type_info, {
	...
	% The (ordered) list of variable definitions (e.g. [{var, FileLoc, 'X' }])
	% of this type:
	%
	...
	% Tells whether this type has been exported, as a (possibly empty) list of
	% the location(s) of its actual export(s), knowing that a type can be
	% exported more than once, or never:
	%
...
-record( function_info, {
	...
	% source file) of this function:
	%
	% (this information is a priori redundant with the one in the first clause,
	...
	% -callback attribute) or not.
	%
	% The latter case is the default (whether this function is declared with a
	% -spec attribute, or with no related attribute at all).
	%
	callback = false :: boolean(),


	% Tells whether this function has been exported, as a (possibly empty) list
	% of the location(s) of its actual export(s), knowing that a function can be
	% exported more than once or never.
	%
	% Note that, provided a legit location is specified here (i.e. the location
	% of an actual export form), this function will be automatically declared in
	% that export form, should it be not already)
	%
	exported = [] :: [ ast_info:ast_location() ] } ).
