...
-moduledoc """
This module allows to generate **modules sharing read-only two-way (bijective)
associative tables whose stored pairs can be read from any number (potentially
extremely large) of readers very efficiently** (possibly the most efficient way
in Erlang).

They are especially useful in order to share a single, larger table (specific to
the generated module), moreover with a low overhead. If multiple tables
...
be of any permanent (non-transient) type (not atoms only; the first elements can
be heterogeneous, they do not have to be of the same type, and the same of
course applies for the second elements). Specifying a transient type will result
in a badarg. As two-way conversions are to be performed, for a given set of
values (first ones or second ones), there must be no duplicated elements
(otherwise no bijectivity can exist).
...
element (first or second) must be found in the table (otherwise an exception is
thrown)
...
on-the-fly, exporting two functions designed to access either of the elements of
...
For that these two 1-arity functions are generated and exported in that module,
as if we were using a bijective_table() underneath.
