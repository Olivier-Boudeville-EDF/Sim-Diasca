...
# Reliable, to avoid being affected by still in-progress developments:

#cd ${CEYLAN_MYRIAD_RELIABLE} && make -s all 1>/dev/null && cd src/apps/generate-password && generate-password.escript $*

#cd ${CEYLAN_MYRIAD_RELIABLE} && make -s all && cd src/apps/generate-password && generate-password.escript $*

# Now not triggering a prior recompilation check, expecting generally to be
# already built:
#
cd ${CEYLAN_MYRIAD_RELIABLE} && cd src/apps/generate-password && generate-password.escript $*
